...
function Colorgame() {
  ...
  const handleCountdownComplete = (tabId, periodId) => {
    // Generate a random winning number (0-9)
    ...
    if (pendingBetsJson) {
      try {
        ...
        const winningBets = processedGameResults.filter((result) => result.isWin);
    ...
    if (processedGameResults && processedGameResults.length > 0) {
      ...
      const userBetHistory = JSON.parse(localStorage.getItem(`userBets_tab${tabId}`) || '[]');
      const updatedUserBetHistory = [...processedGameResults, ...userBetHistory];
      localStorage.setItem(`userBets_tab${tabId}`, JSON.stringify(updatedUserBetHistory));
    }
  };
